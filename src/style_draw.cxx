...
struct RGBAf { float r = 0.f, g = 0.f, b = 0.f, a = 1.f; };

[[nodiscard]] static bool parseHexColor(std::string_view s, RGBAf& out) noexcept
{
...
[[nodiscard]] static float toFloat(std::string_view s) noexcept
{
...
[[nodiscard]] static std::size_t utf8Len(std::string_view s) noexcept
{
    std::size_t n = 0;
...
absolutePos(const RenderTree& tree, NodeHandle handle) noexcept
{
    float ax = 0.f, ay = 0.f;
...
static void walkTree(RenderTree& tree, NodeHandle h,
...
void bindDrawCallbacks(RenderTree& tree, NodeHandle root)
{
    walkTree(tree, root, [&tree](NodeHandle h, RenderNode& n) {
        ...
        const bool hasBorder = !n.style("borderWidth").empty()
                               && !n.style("borderColor").empty();
        const bool hasClip   = (n.layout_props.overflow
        ...
        const bool hasSrc    = !n.style("src").empty();
                 ...
                 (RenderContext& ctx)
        {
            ...
            if (hasBg) {
                ...
                if (!bgSv.empty() && parseHexColor(bgSv, bg) && bg.a * op > 0.001f)
                    ctx.drawRect(ax, ay, self->w, self->h,
                                 bg.r, bg.g, bg.b, bg.a * op);
            }
            ...
            if (hasSrc) {
                ...
                if (!src.empty()) {
                    ...
                    if (hasShader && !shdr.empty()) {
                        ...
                        sp.u_time = ctx.time;
                        ctx.drawImageWithShader(src, shdr,
                                                ax, ay, self->w, self->h,
                                                op, self->visual_props.object_fit,
                                                sp);
                    } else {
                        ctx.drawImage(src, ax, ay, self->w, self->h, op,
            ...
            // Re-read each frame (supports animated border width/color later).
            {
                ...
                if (!bwSv.empty() && !bcSv.empty()) {
                    ...
                    if (parseHexColor(bcSv, bc)) {
                        ...
                        if (bw > 0.f)
                            ctx.drawRectOutline(ax, ay, self->w, self->h, bw,
                                                bc.r, bc.g, bc.b, bc.a * op);
                    }
                }
            }
            ...
            if (!txt.empty()) {
                ...
                switch (vp.text_align) {
                    ...
                    default: // Center — preserves historical centred-label behaviour
                        tx = ax + vp.padding_left
                             + std::max(0.f, inner_w - tw) * 0.5f;
                        break;
                }

                ctx.drawText(txt, tx, ty, fs,
                             fg.r, fg.g, fg.b, fg.a * op);
            }
        };
        ...
        // as-is (node_events.cxx uses its own local compose() helper).
    });
}
