...
static void parseAttrs(std::string_view s, std::size_t &i, std::vector<Token> &out) {
...
std::vector<Token> Lexer::tokenize() const {
    ...
    while (!src.empty()) {
        ...
        std::string_view line = (nl_pos == std::string_view::npos) ? src : src.substr(0, nl_pos);
        src = (nl_pos == std::string_view::npos) ? std::string_view{} : src.substr(nl_pos + 1);
        ...
        std::string_view effective_content = content;
        {
            ...
            if (depth > 0) {
                ...
                while (!src.empty() && depth > 0) {
                    ...
                    auto cont_raw =
                        (cont_nl == std::string_view::npos) ? src : src.substr(0, cont_nl);
                    src = (cont_nl == std::string_view::npos) ? std::string_view{}
                    ...
                    if (cont_content.size() >= 2 && cont_content[0] == '/'
                        && cont_content[1] == '/')
                        continue;
