...
[[nodiscard]]
static bool hasClassToken(std::string_view class_sv, const std::string &token) noexcept {
    std::size_t pos = 0;
    while (pos < class_sv.size()) {
        const std::size_t end = class_sv.find(' ', pos);
        const std::size_t len = (end == std::string_view::npos) ? class_sv.size() - pos : end - pos;
...
static bool isSimpleSelector(std::string_view sel) noexcept {
    ...
    const std::string_view base =
        (colon == std::string_view::npos) ? sel.substr(1) : sel.substr(1, colon - 1);
...
StyleSheet parse(std::string_view source) {
    ...
    for (std::size_t pos = 0;;) {
        ...
        std::fill(
            s.begin() + static_cast<std::ptrdiff_t>(open),
    ...
    while (i < s.size()) {
        ...
        const std::string_view sel_sv = trimSV(std::string_view(s).substr(i, brace_open - i));
        const std::string_view block_sv =
            std::string_view(s).substr(brace_open + 1, brace_close - brace_open - 1);

        i = brace_close + 1;

        // --- Patched for FrameGraph support: allow complex selectors ---
        ...
        if (colon_pos != std::string_view::npos) {
            rule.selector = std::string(sel_sv.substr(0, colon_pos));
            rule.pseudo   = std::string(sel_sv.substr(colon_pos + 1));
        } else {
            rule.selector = std::string(sel_sv);
            rule.pseudo   = "";
        }
        ...
        while (!remaining.empty()) {
            ...
            const std::string_view decl =
                (semi == std::string_view::npos) ? remaining : remaining.substr(0, semi);
            remaining =
                (semi == std::string_view::npos) ? std::string_view{} : remaining.substr(semi + 1);
 ...
 * @return StyleSheet result
 */
static StyleSheet loadRecursive(const std::string &path, std::unordered_set<std::string> &visited) {
    ...
    for (std::size_t i = 0; i < src.size();) {
        if (src[i] == '@' && src.compare(i, 7, "@import") == 0) {
            ...
            const std::string_view stmt = std::string_view(src).substr(i, end - i + 1);
            ...
            if (q1 != std::string_view::npos) {
                ...
                if (q2 != std::string_view::npos) {
                    ...
                    const std::string abs =
                        std::filesystem::path(rel).is_absolute() ? rel : (base_dir / rel).string();
...
void StyleSheet::applyTo(RenderTree &tree, NodeHandle root, float px_scale) {
    ...
    walkTree(tree, root, [this, px_scale](NodeHandle /*h*/, RenderNode &n) {
        ...
        for (const Rule &rule : rules) {
            ...
            bool matched = false;
            if (rule.is_id) {
                // Remove leading '#' for ID comparison.
                matched =
                    !id_sv.empty() && rule.selector.size() > 1 && id_sv == rule.selector.substr(1);
            } else {
                // Must be a class selector (starts with '.').
                if (rule.selector.size() > 1 && rule.selector[0] == '.') {
                    matched = !class_sv.empty() && hasClassToken(class_sv, rule.selector.substr(1));
                }
            }
...
void StyleSheet::tickHover(RenderTree &tree, float phys_x, float phys_y, float px_scale) {
    for (HoverEntry &entry : hover) {
        ...
        const bool now = (phys_x >= ax && phys_x < ax + n->w && phys_y >= ay && phys_y < ay + n->h);

        if (now == entry.hovered)
            continue;
        entry.hovered = now;
    ...
    walkTree(tree, root, [this, &tree](NodeHandle h, RenderNode &n) {
        ...
        for (NodeHandle c = n.child; c.valid();) {
            ...
            if (cn) {
                ...
                for (const Rule &rule : rules) {
                    if (rule.pseudo != "active")
                        continue;

                    bool matched = false;
                    if (rule.is_id) {
                        matched = !id_sv.empty() && rule.selector.size() > 1
                               && id_sv == rule.selector.substr(1);
                    } else {
                        if (rule.selector.size() > 1 && rule.selector[0] == '.') {
                            matched = !class_sv.empty()
                                   && hasClassToken(class_sv, rule.selector.substr(1));
                        }
                    }
                    if (!matched)
                        continue;
                    ...
                    for (const auto &[attr, active_val] : rule.props) {
                        ...
                        for (const Rule &base : rules) {
                            if (!base.pseudo.empty())
                                continue;
                            bool bm = false;
                            if (base.is_id) {
                                bm = !id_sv.empty() && base.selector.size() > 1
                                  && id_sv == base.selector.substr(1);
                            } else {
                                if (base.selector.size() > 1 && base.selector[0] == '.') {
                                    bm = !class_sv.empty()
                                      && hasClassToken(class_sv, base.selector.substr(1));
                                }
                            }
                            if (!bm)
                                continue;
...
void StyleSheet::activateNode(RenderTree &tree, NodeHandle clicked, float px_scale) {
    ...
    for (ActiveEntry &entry : active_entries) {
        ...
        if (should_activate == entry.active)
            continue;  // no change
