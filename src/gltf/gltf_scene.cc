 ...
 * @param tree_root  Red channel component [0, 1]
 * @param base_path  Filesystem path
 *
...
void GltfScene::drawEntry(
    ...
    // 'color' → base_color rgba (CSS override first; fall back to glTF material seed)
    ...
    fg::iterateSceneNodes(
        ...
        [&](fg::Node &node, fg::math::fmat4x4 world_mat_fg) {
            ...
            for (std::size_t prim_idx = 0; prim_idx < mesh.primitives.size(); ++prim_idx) {
                ...
                // Seed colour and PBR defaults from the glTF material using
                // internal --gltf-* keys so that css::StyleSheet::applyTo()
                // can freely override 'color', '--roughness', '--metallic'
                // without being blocked by the !n.hasStyle() CSS guard.
                // drawEntry() checks the CSS key first and falls back to the
                // --gltf-* fallback when no CSS override is present.
                {
                    char buf[32];
                    ...
                    proxy->setStyle("--gltf-roughness", std::string(buf));

                    std::snprintf(buf, sizeof(buf), "%.6f", mat_metal);
                    proxy->setStyle("--gltf-metallic", std::string(buf));
                }
