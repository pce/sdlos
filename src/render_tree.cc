...
std::
    ...
    const std::string_view ext = (backend == GPUBackend::Metal) ? ".metallib" : ".spv";

    const std::string path = std::format("assets/shaders/{}{}", name, ext);

    if (!fs::exists(path)) {
        return std::unexpected(std::format("[loadShader] file not found: {}", path));
    }

    std::ifstream file(path, std::ios::binary | std::ios::ate);
    if (!file) {
        return std::unexpected(std::format("[loadShader] cannot open: {}", path));
    }
    ...
    if (!file.read(
        ...
        return std::unexpected(std::format("[loadShader] read error: {}", path));
    }

    const SDL_GPUShaderFormat fmt =
        (backend == GPUBackend::Metal) ? SDL_GPU_SHADERFORMAT_MSL : SDL_GPU_SHADERFORMAT_SPIRV;
    ...
    if (!shader) {
        return std::unexpected(
            std::format(
                "[loadShader] SDL_CreateGPUShader failed for '{}': {}",
                path,
                SDL_GetError()));
    }
...
void RenderContext::drawImage(
    ...
    if (fit == OF::Contain && img.width > 0 && img.height > 0) {
        ...
        const float img_ar  = static_cast<float>(img.width) / static_cast<float>(img.height);
    ...
    const RectUniform vu{dx, dy, dw, dh, viewport_w, viewport_h, uv_x, uv_y, uv_w, uv_h, 0.f, 0.f};
    SDL_PushGPUVertexUniformData(cmd, 0, &vu, sizeof(vu));
 ...
 */
void RenderContext::drawImageWithShader(
    std::string_view src,
 ...
 * @param shader_params  Opaque resource handle
 */
void RenderContext::drawVideoWithShader(
    std::string_view shader_name,
    float x,
    float y,
    float w,
 ...
 * @param a          Alpha channel component [0, 1]
 */
void RenderContext::drawRectOutline(
    float x,
    float y,
    float w,
    float h,
...
std::span<RenderNode *> collectChildren(RenderNode &n, RenderTree &tree, core::frame_arena &arena) {
    std::size_t count = 0;
...
void flexLayout(RenderNode &n, RenderTree &tree, core::frame_arena &arena, bool is_column) {
    ...
    // Pass 0: resolve percent cross-axis sizes.
    // width_pct / height_pct are the "% of parent dimension" analogs to
    // layout_props.width / height.  Cross-axis percent is resolved here
    // (before align-items kicks in) so that e.g. `width: 100%` on a
    // FlexColumn child reliably fills the container even when alignItems
    // is not Stretch.
    for (RenderNode *child : children) {
        if (is_column) {
            if (child->layout_props.width_pct >= 0.f)
                child->w = container_cross * (child->layout_props.width_pct / 100.f);
        } else {
            if (child->layout_props.height_pct >= 0.f)
                child->h = container_cross * (child->layout_props.height_pct / 100.f);
        }
    }
    ...
    for (RenderNode *child : children) {
        ...
        // Percent main-axis size (e.g. height: 50% on a FlexColumn child).
        if (basis < 0.f) {
            if (is_column && lp.height_pct >= 0.f)
                basis = container_main * (lp.height_pct / 100.f);
            else if (!is_column && lp.width_pct >= 0.f)
                basis = container_main * (lp.width_pct / 100.f);
        }
    ...
    const float free_space = std::max(0.f, container_main - total_content - total_gap_px);
    ...
    for (RenderNode *child : children) {
        ...
        case A::End:
            if (is_column)
                child->x = pad_cross_start + container_cross - child->w;
            else
                child->y = pad_cross_start + container_cross - child->h;
            break;
        case A::Stretch:
            // Only auto-stretch when no explicit width/height or percent was set.
            if (is_column) {
                child->x = pad_cross_start;
                if (child->layout_props.width_pct < 0.f && child->layout_props.width < 0.f)
                    child->w = container_cross;
            } else {
                child->y = pad_cross_start;
                if (child->layout_props.height_pct < 0.f && child->layout_props.height < 0.f)
        ...
        if (child->x != old_x || child->y != old_y || child->w != old_w || child->h != old_h) {
...
void blockLayout(RenderNode &n, RenderTree &tree) {
    ...
    for (NodeHandle c = n.child; c.valid();) {
        ...
        if (child->layout_props.width_pct >= 0.f) {
            // Percent width relative to available content width.
            child->w = avail_w * (child->layout_props.width_pct / 100.f);
            child->x = vp.padding_left;
        } else if (child->layout_props.width < 0.f) {  // -1 = auto: fill content area
            child->w = avail_w;
            child->x = vp.padding_left;
        } else {
            // Fixed-width: left-align for LTR, right-align for RTL.
            child->x = rtl ? (n.w - vp.padding_right - child->w) : vp.padding_left;
        }
        child->y  = y_offset;
 ...
 */
void RenderTree::appendChildren(NodeHandle parent, std::span<const NodeHandle> children) {
...
void RenderTree::markLayoutDirty(NodeHandle handle) {
    for (NodeHandle h = handle; h.valid();) {
        ...
        if (!n)
            break;
        if (n->dirty_layout)
            break;  // Optimization: parent is already dirty, stop propagation.
        n->dirty_layout = true;
        n->dirty_render = true;
        h               = n->parent;
    }
 ...
 * @return Integer result; negative values indicate an error code
 */
std::vector<NodeHandle> RenderTree::findByClass(NodeHandle start, std::string_view cls) const {
    ...
    const auto hasToken = [](std::string_view list, std::string_view tok) noexcept {
        ...
        while (pos <= list.size()) {
            ...
            const std::size_t len = (end == std::string_view::npos) ? list.size() - pos : end - pos;
            if (list.substr(pos, len) == tok)
                return true;
...
void RenderTree::traverse(NodeHandle start, Fn &&fn) {
    ...
    while (!stack.empty()) {
        ...
        RenderNode *n = nodes_.get(h);
        if (!n)
            continue;  // stale handle — skip gracefully
