...
constexpr DebugColor colorForKind(LayoutKind kind) noexcept {
    ...
    case LayoutKind::None:
        return {0.55f, 0.55f, 0.58f};  // gray
    case LayoutKind::Block:
        return {1.00f, 0.55f, 0.10f};  // orange
    case LayoutKind::FlexRow:
        return {0.15f, 0.50f, 1.00f};  // blue
    case LayoutKind::FlexColumn:
        return {0.10f, 0.80f, 0.32f};  // green
    case LayoutKind::Grid:
        return {0.75f, 0.20f, 0.90f};  // purple
    }
...
constexpr const char *justifyAbbrev(LayoutProps::Justify j) noexcept {
    switch (j) {
    case LayoutProps::Justify::Start:
        return "S";
    case LayoutProps::Justify::Center:
        return "C";
    case LayoutProps::Justify::End:
        return "E";
    case LayoutProps::Justify::SpaceBetween:
        return "SB";
    case LayoutProps::Justify::SpaceAround:
        return "SA";
    }
    return "?";
}
...
void drawLabel(
    ...
    ctx.drawRect(lx - pad_h, ly - pad_v, bw, bh, 0.04f, 0.04f, 0.05f, 0.82f);
...
void drawLayoutDebug(
    ...
    while (!stack.empty()) {
        ...
        if (should_draw) {
            ...
            ctx.drawRect(ax, ay, n->w, n->h, c.r, c.g, c.b, cfg.fill_alpha);

            ctx.drawRectOutline(ax, ay, n->w, n->h, cfg.border_width, c.r, c.g, c.b, 0.90f);
