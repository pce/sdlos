 ...
 * @brief Subscribes to
 *
 * @param event_type  Discriminator tag or type enumeration
 * @param callback    Callable invoked on the named event
 */
        ...
        vec.erase(
            std::remove_if(
                ...
                [&callback](const std::function<void(const std::string &)> &existing_callback) {
                    ...
                    auto existing_ptr =
                        existing_callback.template target<void (*)(const std::string &)>();
