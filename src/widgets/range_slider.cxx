// ---------------------------------------------------------------------------
// range_slider.cxx — RangeSlider widget implementation
//
// Part of sdlos — SDL3 GPU render core (C++23).
//
// See range_slider.hh for the public API and usage examples.
// ---------------------------------------------------------------------------

#include "range_slider.hh"
...
// Mirrors the identical helper in input_text_box.cxx and style_draw.cxx.
...
[[nodiscard]] static float snapToStep(float v,
                                      float min_v,
                                      float max_v,
                                      float step) noexcept
{
    const float clamped = std::clamp(v, min_v, max_v);
    if (step <= 0.f) return clamped;
...
[[nodiscard]] static int decimalPlaces(float step) noexcept
{
    if (step <= 0.f) return 2;          // continuous — show 2 dp by default
    if (step >= 1.f) return 0;          // integer steps
    float s = step;
    int   dp = 0;
...
[[nodiscard]] static std::string fmtValue(float v, float step) noexcept
{
    char buf[32];
...
parseFloat(std::string_view sv) noexcept
{
...
static bool applyValue(RenderTree& tree, NodeHandle h,
                       RangeSliderState& s, float raw) noexcept
{
    const float next = snapToStep(raw, s.cfg.min, s.cfg.max, s.cfg.step);
    if (next == s.value) return false;
...
[[nodiscard]] static float thumbCX(const RangeSliderState& s, float ax) noexcept
{
    return ax + s.fraction() * s.trackW();
}

// Map a screen x to a slider raw value (before snap).
[[nodiscard]] static float xToValue(const RangeSliderState& s,
                                    float ax, float screen_x) noexcept
{
...
[[nodiscard]] static bool hitTrack(const RangeSliderState& s,
                                   float ax, float ay,
                                   float mx, float my) noexcept
{
    ...
    return mx >= ax        && mx <= ax + tw
        && my >= cy - tr   && my <= cy + tr;
}

// Hit-test: does (mx, my) fall on the thumb square specifically?
[[nodiscard]] static bool hitThumb(const RangeSliderState& s,
                                   float ax, float ay,
                                   float mx, float my) noexcept
{
...
static void installSliderCallbacks(RenderTree& tree, NodeHandle h,
                                   std::shared_ptr<RangeSliderState> st)
{
    ...
    n->draw = [st, h, &tree](RenderContext& ctx)
    {
        ...
        if (!self || self->w <= 0.f || self->h <= 0.f) return;
        ...
        ctx.drawRect(ax, track_y, tw, c.track_h,
                     c.track_bg.r, c.track_bg.g,
                     c.track_bg.b, c.track_bg.a);

        // ── Filled portion (left of thumb) ───────────────────────────────────
        const float fill_w = s.fraction() * tw;
        if (fill_w > 0.5f) {
            ctx.drawRect(ax, track_y, fill_w, c.track_h,
                         c.track_fill.r, c.track_fill.g,
                         c.track_fill.b, c.track_fill.a);
        }

        // ── Track end caps (thin vertical tick at min and max) ───────────────
        const float cap_h = c.track_h + 4.f;
        const float cap_y = cy - cap_h * 0.5f;
        ctx.drawRect(ax,           cap_y, 1.f, cap_h,
                     c.track_bg.r, c.track_bg.g, c.track_bg.b, c.track_bg.a * 0.6f);
        ctx.drawRect(ax + tw - 1.f, cap_y, 1.f, cap_h,
                     c.track_bg.r, c.track_bg.g, c.track_bg.b, c.track_bg.a * 0.6f);
        ...
        ctx.drawRect(tx - tr - 1.f, cy - tr - 1.f,
                     tr * 2.f + 2.f, tr * 2.f + 2.f,
                     0.f, 0.f, 0.f, 0.30f);

        // Thumb body.
        ctx.drawRect(tx - tr, cy - tr, tr * 2.f, tr * 2.f,
                     tc.r, tc.g, tc.b, tc.a);
        ...
        ctx.drawRect(tx - 1.f, cy - 1.f, 2.f, 2.f,
                     0.f, 0.f, 0.f, 0.25f);
        ...
        if (c.show_value) {
            ...
            ctx.drawText(val_str, lx, ly, c.font_size,
                         c.value_color.r, c.value_color.g,
                         c.value_color.b, c.value_color.a);
        }

        // ── Focus ring ────────────────────────────────────────────────────────
        // Drawn as a 1-px outline one pixel outside the widget bounds.
        if (s.focused) {
            const Color& f = c.track_fill;
            ctx.drawRectOutline(ax - 2.f, ay - 2.f,
                                c.w + 4.f, c.h + 4.f, 1.f,
                                f.r, f.g, f.b, 0.55f);
        }
    };
    ...
    // ── update ───────────────────────────────────────────────────────────────
    // Keep re-queuing dirty_render while dragging or focused so the cursor
...
void RangeSlider::stepDown(int multiplier)
{
    RangeSliderState* s = getState();
    if (!s) return;
    const float effective = s->cfg.step > 0.f
        ? s->cfg.step
        : (s->cfg.max - s->cfg.min) * 0.01f;
    applyValue(tree, handle, *s,
               s->value - effective * static_cast<float>(multiplier));
}
...
bool RangeSlider::handleEvent(const SDL_Event& ev)
{
    ...
    // ── Mouse button down: click-to-seek + begin drag ─────────────────────
    ...
    case SDL_EVENT_MOUSE_BUTTON_UP: {
        if (ev.button.button != SDL_BUTTON_LEFT || !s->dragging) return false;
        s->dragging     = false;
        n->dirty_render = true;
        return true;
    }
    ...
    case SDL_EVENT_KEY_DOWN: {
        ...
        const float eff_step = s->cfg.step > 0.f
            ? s->cfg.step
            : (s->cfg.max - s->cfg.min) * 0.01f;
        ...
        case SDLK_PAGEDOWN:
            ...
            applyValue(tree, handle, *s,
                       s->value - (s->cfg.max - s->cfg.min) * 0.1f);
            return true;
...
static void walkBindInputs(RenderTree& tree, NodeHandle root)
{
    ...
    if (const auto v = parseFloat(n->style("min")))    cfg.min   = *v;
    if (const auto v = parseFloat(n->style("max")))    cfg.max   = *v;
    if (const auto v = parseFloat(n->style("value")))  cfg.value = *v;
    if (const auto v = parseFloat(n->style("step")))   cfg.step  = *v;
    ...
    if (const auto v = parseFloat(n->style("fontSize")))   cfg.font_size = *v;
    if (const auto v = parseFloat(n->style("labelWidth"))) cfg.label_w   = *v;

    {
        const std::string_view sv = n->style("showValue");
        if (sv == "true" || sv == "1") cfg.show_value = true;
    }
