...
static std::size_t utf8Next(std::string_view s, std::size_t pos) noexcept {
    if (pos >= s.size())
        return s.size();
...
static std::size_t utf8Prev(std::string_view s, std::size_t pos) noexcept {
    if (pos == 0)
        return 0;
    const char *it    = s.data() + pos;
...
static bool isValidUtf8(const char *data, std::size_t len) noexcept {
...
static bool insertText(
    ...
    if (chars.empty())
        return false;
    if (max_length > 0 && text.size() + chars.size() > max_length)
        return false;
...
/// Selection helpers
static bool hasSelection(const TextFieldState &s) noexcept {
    return s.sel_active && s.sel_start != s.sel_end;
}

/// Always returns [lo, hi] regardless of anchor direction.
static std::pair<std::size_t, std::size_t> normalizedSel(const TextFieldState &s) noexcept {
    if (!s.sel_active || s.sel_start == s.sel_end)
        return {s.cursor_pos, s.cursor_pos};
    return {std::min(s.sel_start, s.sel_end), std::max(s.sel_start, s.sel_end)};
}
...
static void pushUndo(TextFieldState &s) {
    if (!s.undo_stack.empty() && s.undo_stack.back() == s.text)
        return;
...
static bool performUndo(TextFieldState &s) noexcept {
...
static void replaceSelection(TextFieldState &s, std::string_view replacement) {
    if (hasSelection(s)) {
        ...
        if (s.cfg.max_length > 0 && after_size > s.cfg.max_length)
            return;
        s.text.erase(a, b - a);
...
// Drawing utilities
static void
drawBorderRect(RenderContext &ctx, float x, float y, float w, float h, const Color &c) noexcept {
...
[[nodiscard]]
static constexpr float charWidth(float font_size) noexcept {
...
static void hintImeRect(float screen_x, float screen_y, float font_h) noexcept {
    ...
    const SDL_Rect r{
        ...
        1,
        static_cast<int>(font_h)};
    SDL_SetTextInputArea(w, &r, 0);
}

[[nodiscard]]
static float normWheel(float raw, SDL_MouseWheelDirection dir) noexcept {
...
[[nodiscard]]
static LinePos byteToLinePos(std::string_view text, std::size_t byte_off) noexcept {
    int line        = 0;
    std::size_t col = 0;
...
static std::size_t
linePosToByteOff(std::string_view text, int target_line, std::size_t target_col) noexcept {
    int line      = 0;
    std::size_t i = 0;
...
struct ScrollbarMetrics {
    float track_x, track_y, track_h, thumb_y, thumb_h;
};

static ScrollbarMetrics scrollbarMetrics(const TextFieldState &s) noexcept {
    ...
    const float ty = range > 0.f ? track_y + (s.scroll_offset_y / range) * (track_h - th) : track_y;
...
static RowRange visibleRows(const TextFieldState &s, int total_lines) noexcept {
...
static float lineColPx(
    const TextFieldState &s,
...
[[nodiscard]]
static std::size_t
lineHitTest(const TextFieldState &s, std::string_view line, float px, float font_size) noexcept {
    if (px <= 0.f || line.empty())
        return 0;
...
TextField makeTextField(RenderTree &tree, TextFieldConfig cfg) {
    if (cfg.multiline) {
        if (cfg.rows > 0)
            cfg.h = static_cast<float>(cfg.rows) * cfg.line_height + cfg.padding.top
                  + cfg.padding.bottom;
        if (cfg.cols > 0)
            cfg.w = static_cast<float>(cfg.cols) * charWidth(cfg.font_size) + cfg.padding.left
                  + cfg.padding.right + cfg.scrollbar_w + 4.f;
    }
    ...
    if (st->cfg.multiline) {
        st->content_w =
            st->cfg.w - st->cfg.scrollbar_w - 4.f - st->cfg.padding.left - st->cfg.padding.right;
    ...
    // TODO refactor extract Draw callback
        ...
        if (c.multiline) {
            ...
            if (nl == 1 && lines[0].empty() && !c.placeholder.empty()) {
                ctx.drawText(
                    ...
                    ay + c.padding.top,
                    c.font_size,
                    c.placeholder_color.r,
                    c.placeholder_color.g,
                    c.placeholder_color.b,
                    c.placeholder_color.a);
            } else {
                const auto [first, last] = visibleRows(s, nl);

                // Cursor's line and column (in bytes from that line's start)
                const auto [cur_line, cur_col] = byteToLinePos(s.text, s.cursor_pos);
                ...
                for (int i = first; i < last; ++i) {
                    ...
                    const float ry = ay + c.padding.top + static_cast<float>(i) * c.line_height
                                   - s.scroll_offset_y;
                    if (ry + c.line_height < ay)
                        continue;
                    if (ry > ay + wh)
                        break;
                    ...
                    if (any_sel && selA < lend && selB > lstart) {
                        ...
                        ctx.drawRect(
                            ax + c.padding.left + x0 - s.scroll_offset_x,
                            ry,
                            x1 - x0,
                            c.line_height,
                            kSelBg.r,
                    ...
                    if (i == cur_line && !s.composition.empty()) {
                        ...
                        const std::string_view suffix = line.substr(std::min(cur_col, line.size()));
                        ...
                        const float comp_w =
                            lineColPx(s, s.composition, s.composition.size(), c.font_size);
                ...
                // Cursor
                if (s.focused && blinkOn() && cur_line >= first && cur_line < last) {
                    const std::string_view cl = lines[static_cast<std::size_t>(cur_line)];
                    ...
                    const float cx = ax + c.padding.left - s.scroll_offset_x + pre_w + comp_cw;
            ...
            const std::string display = c.secure ? std::string(s.text.size(), '*') : s.text;

            // Selection background
            if (hasSelection(s) && !display.empty()) {
                const auto [selA, selB] = normalizedSel(s);
                const float x0 = c.secure ? static_cast<float>(selA) * charWidth(c.font_size)
                                          : lineColPx(s, display, selA, c.font_size);
                const float x1 = c.secure ? static_cast<float>(selB) * charWidth(c.font_size)
                                          : lineColPx(s, display, selB, c.font_size);
                ctx.drawRect(
                    tx + x0 - s.scroll_offset_x,
                    ay + 2.f,
                    x1 - x0,
                    wh - 4.f,
                    kSelBg.r,
            ...
            if (s.text.empty() && !c.placeholder.empty()) {
                ...
                if (!s.composition.empty()) {
                    ...
                    const std::string_view suffix = std::string_view(display).substr(cpos);
                    ...
                    if (!suffix.empty())
                        ctx.drawText(
                            suffix,
                            tx + pre_w + comp_w - s.scroll_offset_x,
                            ty,
                            c.font_size,
                            c.text_color.r,
                            c.text_color.g,
            ...
            if (s.focused && blinkOn()) {
                ...
                const float comp_cw = s.composition.empty()
                                        ? 0.f
                                        : lineColPx(
                                              s,
                                              s.composition,
    ...
    n->update      = [st, np]() {
        ...
        if (s.cfg.multiline) {
            const auto lines = splitLines(s.text);
            s.total_h =
                static_cast<float>(std::max(1, static_cast<int>(lines.size()))) * s.cfg.line_height
            ...
            s.scroll_offset_y =
                std::clamp(s.scroll_offset_y, 0.f, std::max(0.f, s.total_h - s.cfg.h));
            s.scroll_offset_x =
                std::clamp(s.scroll_offset_x, 0.f, std::max(0.f, s.total_w - s.content_w));
        }
        if (s.focused)
            np->dirty_render = true;
    };
...
void TextField::setText(std::string text) {
    ...
    if (s->cfg.value)
        s->cfg.value->set(s->text);
    if (s->cfg.on_change)
        s->cfg.on_change(s->text);
    ...
    if (!s->cfg.multiline) {
        ...
        if (s->text_renderer) {
            const float cursor_px = static_cast<float>(
                s->text_renderer->measureText(s->text.substr(0, s->cursor_pos), s->cfg.font_size)
                    .first);
            const float visible_w = s->cfg.w - s->cfg.padding.left - s->cfg.padding.right;
...
bool TextField::handleEvent(const SDL_Event &ev) {
    ...
    case SDL_EVENT_MOUSE_BUTTON_DOWN: {
        ...
        if (s->cfg.multiline && s->total_h > s->cfg.h) {
            ...
            if (lx >= sb.track_x && lx <= sb.track_x + s->cfg.scrollbar_w && ly >= sb.thumb_y
                && ly <= sb.thumb_y + sb.thumb_h) {
        ...
        if (s->cfg.multiline) {
            ...
            const int cli    = std::max(0, static_cast<int>(ly / s->cfg.line_height));
            const auto lines = splitLines(s->text);
            const int li     = std::min(cli, std::max(0, static_cast<int>(lines.size()) - 1));
            const auto &lsv  = lines[static_cast<std::size_t>(li)];
            const std::size_t lstart = static_cast<std::size_t>(lsv.data() - s->text.data());
    ...
    case SDL_EVENT_MOUSE_MOTION: {
        ...
        if (s->scrollbar_drag) {
            ...
            if (range > 0.f) {
                ...
                s->scroll_offset_y = std::clamp(s->drag_start_off + dy * (sr / range), 0.f, sr);
            }
            tree.markDirty(handle);
            return true;
        }
    ...
    case SDL_EVENT_KEY_DOWN: {
        ...
        if (primary) {
            ...
            const bool is_nav_key = key == SDLK_LEFT || key == SDLK_RIGHT || key == SDLK_UP
                                 || key == SDLK_DOWN || key == SDLK_HOME || key == SDLK_END;
            if (!is_nav_key)
                return true;
        }

        // NOTE: move cursor with optional Shift-based selection
        ...
        if (key == SDLK_BACKSPACE) {
            ...
            if (!shift && hasSelection(*s)) {
                ...
                const std::size_t new_pos =
                    primary ? (s->cfg.multiline ? linePosToByteOff(
                                                      s->text,
                                                      byteToLinePos(s->text, s->cursor_pos).line,
                                                      0)
                                                : std::size_t{0})
            ...
            if (s->cfg.multiline) {
                const auto [line, col] = byteToLinePos(s->text, s->cursor_pos);
                const std::size_t new_pos =
                    primary ? std::size_t{0}
                            : (line > 0 ? linePosToByteOff(s->text, line - 1, col) : s->cursor_pos);
                moveCursor(new_pos);
                scrollToCursor();
            }
            ...
            if (s->cfg.multiline) {
                const auto [line, col] = byteToLinePos(s->text, s->cursor_pos);
                const std::size_t new_pos =
                    primary ? s->text.size() : linePosToByteOff(s->text, line + 1, col);
                moveCursor(new_pos);
                scrollToCursor();
            }
            ...
            const std::size_t new_pos =
                s->cfg.multiline
                    ? linePosToByteOff(s->text, byteToLinePos(s->text, s->cursor_pos).line, 0)
                    : std::size_t{0};
            moveCursor(new_pos);
            if (s->cfg.multiline)
                scrollToCursor();
            ...
            if (s->cfg.multiline) {
                ...
                const auto [li, col]  = byteToLinePos(s->text, s->cursor_pos);
                const std::size_t liu = static_cast<std::size_t>(li);
                new_pos = linePosToByteOff(s->text, li, liu < lines.size() ? lines[liu].size() : 0);
            } else {
                new_pos = s->text.size();
            }
            moveCursor(new_pos);
