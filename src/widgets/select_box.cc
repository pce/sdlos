...
void drawArrow(
    ...
    for (int i = 0; i < steps; ++i) {
        ...
        const float half  = pointing_up ? size * 0.5f * frac           // grows downward
                                        : size * 0.5f * (1.f - frac);  // shrinks downward
        const float row_y = cy + static_cast<float>(i) * 2.f;
        ctx.drawRect(cx - half, row_y, half * 2.f, 2.f, c.r, c.g, c.b, c.a);
    }
}

// 1-pixel border rectangle (top / bottom / left / right strokes).
void drawBorder(RenderContext &ctx, float x, float y, float w, float h, const Color &c) noexcept {
...
[[nodiscard]]
std::size_t hitTestDropdown(const SelectBoxState &s, float lx, float ly) noexcept {
    ...
    if (lx < 0.f || lx > panel_w)
        return SIZE_MAX;
    if (ly < panel_y || ly > panel_y + panel_h)
        return SIZE_MAX;

    const std::size_t idx = static_cast<std::size_t>((ly - panel_y) / s.cfg.item_h);
        ...
        for (std::size_t i = 0; i < s.options.size(); ++i) {
            ...
            if (i == s.selected_idx) {
                ctx.drawRect(
                    ...
                    c.item_selected_bg.g,
                    c.item_selected_bg.b,
            ...
            // Row text
            const Color &tc    = (i == s.selected_idx) ? c.item_selected_text : c.item_text;
...
void SelectBox::selectAt(std::size_t index) {
    SelectBoxState *s = getState();
    if (!s || index >= s->options.size())
        return;
    if (s->selected_idx == index)
...
void SelectBox::open() {
    SelectBoxState *s = getState();
    if (!s || s->is_open || s->options.empty())
        return;
    s->is_open     = true;
...
bool SelectBox::handleEvent(const SDL_Event &ev) {
    ...
    case SDL_EVENT_KEY_DOWN: {
        ...
        if (key == SDLK_UP) {
            const std::size_t cur = (s->hovered_idx != SIZE_MAX) ? s->hovered_idx : s->selected_idx;
            if (cur > 0) {
                s->hovered_idx = cur - 1;
                tree.markDirty(handle);
            }
