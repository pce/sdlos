...
struct RangeSliderState {
    ...
    [[nodiscard]]
    float trackW() const noexcept {
        return cfg.show_value ? cfg.w - cfg.label_w - 8.f  // 8 px gap between track and label
                              : cfg.w;
    }
};
...
struct RangeSlider : WidgetView<RangeSliderState> {
     ...
     */
    [[nodiscard]]
    RangeSliderState *getState() noexcept {
     ...
     */
    [[nodiscard]]
    const RangeSliderState *getState() const noexcept {
        return WidgetView<RangeSliderState>::getState();
    }
