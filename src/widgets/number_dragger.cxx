...
static std::string fmtVal(float v, float step) noexcept {
    ...
    if (step > 0.f && step < 1.f) {
        float s = step;
        while (s < 1.f - 1e-5f && dp < 6) { s *= 10.f; ++dp; }
    }
    char buf[32];
...
static bool applyValue(RenderTree& tree, NodeHandle h,
                       NumberDraggerState& s, float raw) noexcept {
    const float next = snapClamp(raw, s.cfg.min, s.cfg.max, s.cfg.step);
    if (next == s.value) return false;
    s.value = next;
    if (s.cfg.value_signal) s.cfg.value_signal->set(next);
    if (s.cfg.on_change)    s.cfg.on_change(next);
    if (RenderNode* n = tree.node(h)) n->dirty_render = true;
    return true;
}
...
static void installCallbacks(RenderTree& tree, NodeHandle h,
                              std::shared_ptr<NumberDraggerState> st) {
    ...
    n->draw = [st, h, &tree](RenderContext& ctx) {
        ...
        const bool dragging = s.mode == DraggerMode::Dragging
        ...
        if (arrows) {
            ...
            ctx.drawText("<", ax + 3.f, aty, asz,
                c.arrow_color.r, c.arrow_color.g, c.arrow_color.b, c.arrow_color.a);
            ctx.drawText(">", ax + w - asz - 3.f, aty, asz,
                c.arrow_color.r, c.arrow_color.g, c.arrow_color.b, c.arrow_color.a);
        }

        // value / edit text
        const std::string disp = editing ? s.edit_text : fmtVal(s.value, c.step);
        const float tw = ctx.text_renderer
        ...
        ctx.drawText(disp, tx, ty, c.font_size,
            c.text_color.r, c.text_color.g, c.text_color.b, c.text_color.a);

        // cursor blink in edit mode
        if (editing && blinkOn()) {
            const std::string_view pre = std::string_view{s.edit_text}.substr(0, s.cursor_pos);
            const float pw = ctx.text_renderer
                ? static_cast<float>(ctx.text_renderer->measureText(pre, c.font_size).first)
                : static_cast<float>(s.cursor_pos) * c.font_size * 0.55f;
            ctx.drawRect(tx + pw, ty, 1.5f, c.font_size * 1.1f,
                c.cursor_color.r, c.cursor_color.g, c.cursor_color.b, c.cursor_color.a);
        }

        // edit-mode border
        if (editing)
            ctx.drawRectOutline(ax, ay, w, wh, 1.f,
                c.border_edit.r, c.border_edit.g, c.border_edit.b, 0.8f);

        // hover underline
        if (s.hovered && !editing)
            ctx.drawRect(ax + 2.f, ay + wh - 1.f, w - 4.f, 1.f,
                c.arrow_color.r, c.arrow_color.g, c.arrow_color.b, 0.7f);
    };

    n->update = [st, h, &tree]() {
        const bool needs = st->mode != DraggerMode::Idle || st->hovered;
        if (needs)
            if (RenderNode* self = tree.node(h)) self->dirty_render = true;
    };
...
NumberDragger makeNumberDragger(RenderTree& tree, NumberDraggerConfig cfg) {
    ...
    RenderNode*      n = tree.node(h);
    assert(n);
    n->w = cfg.w; n->h = cfg.h; n->layout_kind = LayoutKind::None;
...
bool NumberDragger::isEditing() const noexcept {
    const auto* s = getState(); return s && s->mode == DraggerMode::Editing;
}
...
bool NumberDragger::handleEvent(const SDL_Event& ev) {
    ...
    auto commitEdit = [&] {
        if (s->mode != DraggerMode::Editing) return;
        if (auto p = parseFloat(s->edit_text)) applyValue(tree, handle, *s, *p);
        if (s->cfg.on_commit) s->cfg.on_commit(s->value);
        s->mode = DraggerMode::Idle; s->edit_text = {}; s->cursor_pos = 0;
        stopText(); n->dirty_render = true;
    };
    auto cancelEdit = [&] {
        if (s->mode != DraggerMode::Editing) return;
    ...
    case SDL_EVENT_MOUSE_BUTTON_DOWN: {
        ...
        if (!hit) { commitEdit(); return false; }
        if (s->mode == DraggerMode::Editing) return true;
        s->mode = DraggerMode::PendingDrag;
        s->drag_start_x = mx; s->drag_start_val = s->value;
        n->dirty_render = true;
        return true;
    }
    ...
    case SDL_EVENT_MOUSE_MOTION: {
        ...
        if (in != s->hovered) { s->hovered = in; n->dirty_render = true; }
    ...
    case SDL_EVENT_MOUSE_BUTTON_UP: {
        ...
        if (s->mode == DraggerMode::PendingDrag) {
            ...
            startText(); n->dirty_render = true;
            return true;
        }
        if (s->mode == DraggerMode::Dragging) {
            s->mode = DraggerMode::Idle; n->dirty_render = true; return true;
        }
        return false;
    }

    case SDL_EVENT_TEXT_INPUT: {
        if (s->mode != DraggerMode::Editing) return false;
    ...
    case SDL_EVENT_KEY_DOWN: {
        if (s->mode == DraggerMode::Editing) {
            switch (ev.key.key) {
            case SDLK_RETURN: case SDLK_KP_ENTER: commitEdit(); return true;
            case SDLK_ESCAPE:                      cancelEdit(); return true;
            case SDLK_BACKSPACE:
                if (s->cursor_pos > 0) {
                ...
                return true;
            case SDLK_LEFT:
                if (s->cursor_pos > 0) { --s->cursor_pos; n->dirty_render = true; }
                return true;
            case SDLK_RIGHT:
                if (s->cursor_pos < s->edit_text.size()) { ++s->cursor_pos; n->dirty_render = true; }
                return true;
            default: break;
            }
            return false;
        ...
        const float eff = s->cfg.step > 0.f ? s->cfg.step
        ...
        if (ev.key.key == SDLK_LEFT  || ev.key.key == SDLK_DOWN)
            { applyValue(tree, handle, *s, s->value - inc); return false; }
...
static void walkBindDragNum(RenderTree& tree, NodeHandle root) {
    ...
    if (n->style("tag") != "input" || n->style("type") != "dragnum") return;

    auto pf = [](std::string_view sv) -> std::optional<float> {
        if (sv.empty()) return {};
        float v{}; auto [p, ec] = std::from_chars(sv.data(), sv.data() + sv.size(), v);
        return ec == std::errc{} ? std::optional<float>{v} : std::nullopt;
    };
    ...
    if (auto v = pf(n->style("width")))    { cfg.w = *v; n->w = *v; }
    if (auto v = pf(n->style("height")))   { cfg.h = *v; n->h = *v; }
    if (auto v = pf(n->style("fontSize"))) cfg.font_size = *v;
