...
enum class DraggerMode : uint8_t {
    Idle,
    PendingDrag,
    Dragging,
    Editing
...
struct NumberDraggerState {
     ...
     */
    [[nodiscard]]
    float effectiveDragSpeed() const noexcept {
        if (cfg.drag_speed > 0.f)
            return cfg.drag_speed;
        const float rs = (cfg.max > cfg.min) ? (cfg.max - cfg.min) / 200.f : 0.f;
        const float ss = cfg.step > 0.f ? cfg.step * 0.2f : 0.f;
        if (rs > 0.f && ss > 0.f)
            return std::min(rs, ss);
...
struct NumberDragger : WidgetView<NumberDraggerState> {
     ...
     */
    NumberDraggerState *getState() noexcept { return WidgetView<NumberDraggerState>::getState(); }
    /**
     * @brief Returns state
     *
     * @return Pointer to the result, or nullptr on failure
