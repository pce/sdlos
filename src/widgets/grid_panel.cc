...
static std::size_t hitTest1D(const GridPanelState& s,
                              float ax, float ay,
                              float mx, float my) noexcept
{
    const float lx = mx - ax;
    const float ly = my - ay;
    if (lx < 0.f || ly < 0.f) return SIZE_MAX;
    ...
    if (col < 0 || col >= static_cast<int>(s.cfg.cols)) return SIZE_MAX;
    if (row < 0 || row >= static_cast<int>(s.cfg.rows)) return SIZE_MAX;

    const std::size_t idx =
        static_cast<std::size_t>(row) * static_cast<std::size_t>(s.cfg.cols) +
...
hitTest2D(const GridPanelState& s, float ax, float ay,
          float mx, float my) noexcept
{
    ...
    const float lx = mx - ax;
    const float ly = my - ay;
    if (lx < 0.f || ly < 0.f) return {kNone, kNone};
    ...
    if (c < 0 || c >= static_cast<int>(s.cfg.cols)) return {kNone, kNone};
    if (r < 0 || r >= static_cast<int>(s.cfg.rows)) return {kNone, kNone};
    if (static_cast<std::size_t>(c) >= s.col_values.size()) return {kNone, kNone};
    if (static_cast<std::size_t>(r) >= s.row_values.size()) return {kNone, kNone};
...
static void drawCell1D(RenderContext& ctx,
                       ...
                       float cx, float cy)
{
    ...
    const Color& bg    = sel     ? s.cfg.bg_selected
                       : hovered ? s.cfg.bg_hover
                                 : s.cfg.bg_cell;
    ...
    const float tw = ctx.text_renderer
        ? static_cast<float>(
              ctx.text_renderer->measureText(
                  std::string(lbl), s.cfg.font_size).first)
...
static void drawCell2D(RenderContext& ctx,
                       const GridPanelState& s,
...
static void drawGrid1D(RenderContext& ctx,
                       const GridPanelState& s,
                       float ax, float ay)
{
    const float cw  = s.cfg.cell_w;
...
static void drawGrid2D(RenderContext& ctx,
                       const GridPanelState& s,
                       float ax, float ay)
{
    const float cw  = s.cfg.cell_w;
...
static void installCallbacks(RenderTree& tree, NodeHandle h,
                              std::shared_ptr<GridPanelState> st)
{
    ...
    n->update = [st, h, &tree]() {
        const bool needs = (st->hovered_idx != SIZE_MAX) ||
                           (st->hovered_row != SIZE_MAX);
        if (needs)
...
GridPanel makeGridPanel(RenderTree& tree, GridPanelConfig cfg)
{
    ...
    if (!st->is_2d) {
        st->selected_idx = st->values.empty()
            ? std::size_t(0)
            : std::min(cfg.initial_idx, st->values.size() - 1);
    } else {
...
{
    const auto* s = getState();
    return s ? std::make_pair(s->current_x(), s->current_y())
...
void GridPanel::selectByValue(float value)
{
    ...
    if (best == s->selected_idx) return;
    s->selected_idx = best;

    if (s->cfg.value_signal) s->cfg.value_signal->set(s->current_value());
    if (s->cfg.on_change)    s->cfg.on_change(s->current_value());
    if (RenderNode* n = tree.node(handle)) n->dirty_render = true;
}
...
bool GridPanel::handleEvent(const SDL_Event& ev)
{
    ...
    case SDL_EVENT_MOUSE_MOTION: {
        ...
        if (!s->is_2d) {
            const std::size_t idx =
                hitTest1D(*s, ax, ay, ev.motion.x, ev.motion.y);
            ...
            const auto [r, c] =
                hitTest2D(*s, ax, ay, ev.motion.x, ev.motion.y);
    ...
    case SDL_EVENT_MOUSE_BUTTON_DOWN: {
        ...
        if (!s->is_2d) {
            const std::size_t idx =
                hitTest1D(*s, ax, ay, ev.button.x, ev.button.y);
            ...
            const auto [r, c] =
                hitTest2D(*s, ax, ay, ev.button.x, ev.button.y);
