...
public:
    ...
    /// so HarfBuzz applies Arabic/Hebrew contextual shaping.  The direction
    /// is included in the cache key, so LTR and RTL variants are cached
    /// independently.
    ...
    [[nodiscard]] GlyphTexture ensureTexture(std::string_view text)
    {
        return ensureTexture(text, 0.f, false);
    }
    ...
    /// Pass `rtl=true` to match measurement with an RTL-shaped render.
...
private:
    ...
    struct Key {
        ...
        bool        rtl;    // direction — LTR and RTL variants cached separately

        bool operator==(const Key& o) const noexcept
        {
            return size == o.size && rtl == o.rtl && text == o.text;
        }
    };
    ...
    struct KeyHash {
        ...
        {
            // FNV-1a over text bytes, xor'd with a size hash and direction bit.
