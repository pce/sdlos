...
std::
    expected<Vfs::Dispatch, std::string>
    /**
     * @brief Dispatches
     *
...
std::expected<std::vector<std::byte>, std::string> Vfs::read(std::string_view uri) noexcept {
    auto d = dispatch(uri);
    if (!d)
...
void Vfs::dump_mounts() const noexcept {
    try {
        ...
        for (const auto &[scheme, mount] : mounts_) {
            ...
            std::string desc;
            if (local)
                desc = "LocalMount @ " + local->root().string();
            else if (mem)
                desc = "MemMount (" + std::to_string(mem->size()) + " entries)";
            else
                desc = "IMount (custom)";
