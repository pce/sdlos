...
[[nodiscard]]
static std::optional<uint64_t> toU64(std::string_view s) noexcept {
    if (s.empty())
        return std::nullopt;
    uint64_t v{};
...
static std::function<void()> compose(std::function<void()> a, std::function<void()> b) {
    if (!a)
        return b;
    ...
    walkTree(tree, root, [&](NodeHandle h, RenderNode &n) {
        // Interval timer: fires bus.publish(ontick, "") every interval-ms.
        {
            ...
            if (!interval_sv.empty() && !tick_sv.empty()) {
                ...
                if (ms && *ms > 0) {
                    ...
                    n.update = compose(std::move(n.update), [&bus, state]() mutable {
        ...
        // One-shot reveal: fires once on the first update(), then clears itself.
        {
            ...
            if (!reveal_sv.empty()) {
                ...
                n.update = compose(
                    std::move(n.update),
                    [&tree, h, &bus, topic, fired = std::make_shared<bool>(false)]() mutable {
                        if (*fired)
                            return;
                        *fired = true;
                        bus.publish(topic, "");
