...
class SDLRenderer {
  public:
    ...
    /// Submit a pipeline.pug source string to be compiled on the next frame.
    ///
    /// Engine pattern — deferred rebuild:
    ///   The caller (behavior, DCC tool) builds a pipeline.pug string and calls
    ///   this from any context (event handler, bus callback, UI callback).
    ///   The actual FrameGraph::from_pug() + compile() runs at the TOP of the
    ///   next Render() call — after SDL_AcquireGPUSwapchainTexture, before any
    ///   render pass opens.  This guarantees:
    ///     1. GPU objects are only created on the render thread.
    ///     2. No partial frame: the old pipeline finishes its current frame first.
    ///     3. SDL_CreateGPUGraphicsPipeline is called with a live device and known
    ///        swapchain format.
    ///
    /// Calling this multiple times before the next Render() is safe — only the
    /// last submitted source is compiled (latest wins).
    ///
    /// The source string is moved into internal storage; the caller's copy is
    /// consumed (or can be const ref if the caller needs to keep it).
    void SubmitPipelineSource(std::string source) noexcept;
     ...
     */
    bool IsValid() const noexcept { return initialized_.load(); }
    ...
    /**
     * @brief Sets gpu pre shutdown hook
     *
     * @param hook  Opaque resource handle
     */
