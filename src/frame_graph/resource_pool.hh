...
public:
    ...
    void resize(uint32_t new_w, uint32_t new_h) noexcept {
        if (new_w == sw_w_ && new_h == sw_h_) return;
        sw_w_ = new_w;
        sw_h_ = new_h;
    ...
    SDL_GPUTexture* acquire(std::string_view id) noexcept {
        ...
        // Fast path — already allocated and valid.
        ...
        if (dit == descs_.end()) return nullptr;
