...
[[nodiscard]]
static constexpr std::string_view trim(std::string_view s) noexcept {
...
[[nodiscard]]
static bool starts_with_tag(std::string_view line, std::string_view tag) noexcept {
...
[[nodiscard]]
static StyleMap parse_attrs(std::string_view block) noexcept {
    StyleMap result;
    std::size_t pos = 0;
    ...
    while (pos < n) {
        while (pos < n
               && (block[pos] == ' ' || block[pos] == '\t' || block[pos] == '\r'
                   || block[pos] == '\n'))
            ++pos;
        if (pos >= n)
            break;
        ...
        while (pos < n && block[pos] != '=' && block[pos] != ' ' && block[pos] != '\t'
               && block[pos] != ')')
            ++pos;
        if (pos >= n || block[pos] != '=')
            break;
...
[[nodiscard]]
static std::vector<std::string> split_words(std::string_view s) noexcept {
...
[[nodiscard]]
static TexFormat parse_tex_format(std::string_view s) noexcept {
    if (s == "rgba16f")
        return TexFormat::RGBA16F;
    if (s == "depth32")
        return TexFormat::Depth32F;
    return TexFormat::RGBA8;
}

[[nodiscard]]
static TexSize parse_tex_size(std::string_view s) noexcept {
...
[[nodiscard]]
static uint32_t parse_uint(std::string_view s) noexcept {
    uint32_t v = 0;
...
struct LineParser {
    ...
    [[nodiscard]]
    bool extract_id_and_attrs(std::size_t tag_len, std::string &id_out, StyleMap &attrs_out)
        const noexcept {
        ...
        const auto attr_block = after_tag.substr(paren_open + 1, paren_close - paren_open - 1);
        attrs_out             = parse_attrs(attr_block);
        return true;
    }

    [[nodiscard]]
    std::optional<VariantDesc> try_variant() const noexcept {
        if (!starts_with_tag(line, "variant"))
            return std::nullopt;
        VariantDesc vd;
        StyleMap attrs;
        if (!extract_id_and_attrs(7, vd.id, attrs))
            return std::nullopt;
    ...
    [[nodiscard]]
    std::optional<ResourceDesc> try_resource() const noexcept {
        if (!starts_with_tag(line, "resource"))
            return std::nullopt;
        ResourceDesc rd;
        StyleMap attrs;
        if (!extract_id_and_attrs(8, rd.id, attrs))
            return std::nullopt;
        if (const auto *v = attrs.find("format"))
            rd.format = parse_tex_format(*v);
        if (const auto *v = attrs.find("size"))
            rd.size = parse_tex_size(*v);
        if (const auto *v = attrs.find("w"))
            rd.w = parse_uint(*v);
        if (const auto *v = attrs.find("h"))
            rd.h = parse_uint(*v);
        return rd;
    }
    ...
    [[nodiscard]]
    std::optional<PassDesc> try_pass() const noexcept {
        if (!starts_with_tag(line, "pass"))
            return std::nullopt;
        PassDesc pd;
        StyleMap attrs;
        if (!extract_id_and_attrs(4, pd.id, attrs))
            return std::nullopt;
        ...
        if (const auto *v = attrs.find("shader"))
            pd.shader_key = *v;
        if (const auto *v = attrs.find("writes"))
            pd.writes = *v;
        if (const auto *v = attrs.find("enabled"))
            pd.enabled = parse_bool(*v);
        if (const auto *v = attrs.find("reads"))
            pd.reads = split_words(*v);
...
[[nodiscard]]
std::
    expected<ParseResult, std::string>
    /**
     * @brief Parses
     *
    ...
    while (pos <= src.size()) {
        ...
        const auto raw =
            (end == std::string_view::npos) ? src.substr(pos) : src.substr(pos, end - pos);
        ...
        const bool is_decl = starts_with_tag(line, "pass") || starts_with_tag(line, "resource")
                          || starts_with_tag(line, "variant");

        if (is_decl && line.find('(') != std::string_view::npos
            && line.find(')') == std::string_view::npos) {
            folded_buf = std::string(line);
            while (pos <= src.size() && folded_buf.find(')') == std::string::npos) {
                const auto cont_end = src.find('\n', pos);
                const auto cont_raw = (cont_end == std::string_view::npos)
                                        ? src.substr(pos)
                                        : src.substr(pos, cont_end - pos);
                pos = (cont_end == std::string_view::npos) ? src.size() + 1 : cont_end + 1;
                ++line_no;
