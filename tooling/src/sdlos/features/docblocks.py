...
class Docblocks:
    ...
    def _reflect(
        ...
        **Idempotency guarantee:** This method uses ``_has_preceding_docblock()``
        to filter out functions that already have docblocks, ensuring that
        repeated runs produce the same result (no duplicate docblocks).
...
def _has_preceding_docblock(lines: list[str], decl_line: int) -> bool:
    ...
    nearest non-blank content is a doxygen comment block (either ``/** */`` style
    or ``///`` style).

    **Idempotency:** This function is called before any docblock is inserted,
    so repeated runs will correctly detect previously-inserted blocks and skip them.
    ...
    -------
    bool
        True when a docblock (/** */, ///, or ///**) is already present immediately above.
    """
    ...
    # Check the nearest non-blank line above the declaration.
    line = lines[idx]
    stripped = line.strip()

    # Match various docblock/comment styles:
    # - /** … */ (Doxygen block close)
    # - /// (Doxygen triple-slash, any position)
    # - /** (Doxygen block open)
    # - ///** (Alternative triple-slash variant)
    ...
    # If the nearest non-blank is a continuation of a docblock (* lines),
    # scan upward to find the opening /** marker.
    if stripped.startswith("*") and not stripped.startswith("*/"):
        # Walk further up to find the opening /** or triple slash.
        scan_idx = idx - 1
        while scan_idx >= 0:
            scan_line = lines[scan_idx].strip()
            if not scan_line or scan_line.startswith("*"):
                # Still in the docblock or blank space above it
                scan_idx -= 1
                continue
            if scan_line.startswith("/**") or scan_line.startswith("///") or scan_line.startswith("///**"):
                return True
            # Non-comment line reached; no docblock found
            break
        return False
