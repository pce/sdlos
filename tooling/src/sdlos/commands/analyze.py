...
def _make_pipeline(
    *,
    ...
    from ..features.sanitize import FileIntegrityProcessor
    ...
    # Integrity scan runs first — downstream processors receive clean bytes.
    if run_integrity:
        processors.append(
            FileIntegrityProcessor(
                strip=strip_integrity and write,
            )
        )
...
@click.option(
    ...
    show_default=True,
    help=(
        "Scan for hidden or dangerous bytes: BOM, C0/C1 control characters, "
        "zero-width / invisible Unicode (watermark vectors), BiDi overrides "
        "(Trojan Source — CVE-2021-42574), Unicode tag characters "
        "(U+E0000–E007F), variation selectors, and mixed line endings.  "
        "Use // SANITIZE:SKIP_BEGIN / SANITIZE:SKIP_END in source to exempt "
        "known-good sections."
    ),
)
@click.option(
    ...
    default=False,
    help=(
        "Remove all integrity findings in place (implies --integrity --write).  "
        "BOM stripped, invisible chars deleted, control chars replaced with "
        "space, CRLF normalised to LF.  Lines inside SANITIZE:SKIP ranges "
...
def cmd_analyze(
    ...
    Integrity scan
    --------------
    --integrity (on by default) detects BOM, C0/C1 control characters,
    zero-width / invisible Unicode, BiDi overrides (CVE-2021-42574 Trojan
    Source), Unicode tag characters (U+E0000-E007F, a lossless watermark
    alphabet), and mixed CRLF/LF line endings.

    Use // SANITIZE:SKIP_BEGIN ... // SANITIZE:SKIP_END comment markers to
    exclude known-good sections (e.g. UI string literals with Unicode).
    Pass --strip-integrity --write to remove all flagged characters in place.
    """
    try:
    ...
    pipeline = _make_pipeline(
        ...
        strip_integrity=strip_integrity,
    ...
    if not pipeline.processors:
        raise click.UsageError(
            ...
            "  --sca, --tidy, --format, --docblocks, --metrics, --integrity"
        )
